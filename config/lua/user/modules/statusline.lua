...
M.setup = function()
  ...
  local function mode_segment()
    ...
    return ' '
      .. hl_str(hl, ' ' .. (map[mode] or mode:sub(1, 1):upper()) .. ' ')
  end
  ...
  local function diag_str()
    ...
    if total[2] then
      insert(
        parts,
        hl_str('DiagnosticWarn', icons.diagnostics.WARN .. total[2])
      )
    end
    if total[3] then
      insert(
        parts,
        hl_str('DiagnosticInfo', icons.diagnostics.INFO .. total[3])
      )
    end
    if total[4] then
      insert(
        parts,
        hl_str('DiagnosticHint', icons.diagnostics.HINT .. total[4])
      )
    end
