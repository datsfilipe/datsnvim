{
  ...
  outputs = { self, nixpkgs, flake-utils, hlchunk, wakatime }: 
    let
      defaultConfig = {
        theme = "default";
        lazy.lock = "default";
      ...
      let
        mkNeovimConfig = config:
          let
            ...
            '';
            initTheme = pkgs.writeText "init.lua" ''
              return ${config.theme}
            '';
          ...
          in pkgs.symlinkJoin {
            ...
            paths = [
              ...
              (if config.theme != "default" then pkgs.writeTextDir "lua/extensions/colorschemes/init.lua"
                (builtins.readFile initTheme) else "")
              (if config.lazy.lock != "default" then pkgs.writeTextDir "lua/extensions/lazy/init.lua"
                (builtins.readFile initLazy) else "")
              ./.
            ];
          };
      in {
